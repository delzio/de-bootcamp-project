...
# NOTE: Can take several minutes depending on internet speed
def batch_context_to_gcs(gcs_bucket, gcs_input_path, gcs_output_path):
    # Start spark session
...
def processed_to_bq(gcs_bucket, dataset, gcs_raw_path, gcs_sample_path, project_id, current_time, full_backfill=False):
    # Start spark session
    spark = start_spark_session()
...
def pls_prediction_to_bq(gcs_bucket, dataset, gcs_raw_path, project_id, pls_model_path, current_time):
    # Start spark session
    spark = start_spark_session()
    logging.info("spark session created")
    ...
    pls_model = joblib.load(pls_model_path)
